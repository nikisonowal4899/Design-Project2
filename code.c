...
#define incorrect_attempts 0xC6
...
void main() {
	...
	data_index = 0;
	security_breach = eeprom_read_byte(incorrect_attempts);
...
void keyboard_input(unsigned char dat) {
	...
	if(dat == '#') { // Enter is pressed // Enter key will have many purpose		
		if(data_index != 0 && screen_flag == 'H') { // Password matching purpose 
			...
			if(check == '1') {
				...
				screen_flag = 'R';
				security_breach = 0;
				eeprom_write_byte(incorrect_attempts, security_breach);
			} else if(check == '0') {
				lcd_print(0x20); // wrong password
				screen_flag = 'X';
				security_breach++;
				if(security_breach == 4) buzzer = 1;
					else eeprom_write_byte(incorrect_attempts, security_breach);
			} 
		}
	...
	else if(dat == 'D') { // Home key is pressed // Home key need not require any Flag to check 
		unsigned char i;
		home_screen();
